...
exports.investFor = function(username, new_balance, investment, callback) {
  exports.load(function(storage) {
    DB.users[username].balance = new_balance;
    if(DB.users[username][investment.identifier]) {
      DB.users[username][investment.identifier].num_shares += investment.num_shares;
    } else {
      DB.users[username][investment.identifier] = investment;
    }
    exports.store(function() {
      callback(DB.users[username]);
    });
  });
};
